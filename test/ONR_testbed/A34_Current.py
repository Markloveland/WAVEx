...
This algorithm for loading will be same required of Action Balance Equation
    du/dt + \/.cu = f
Case from ONR Testbed case A34
"""
...
theta_min = np.pi/2 - 70/180*np.pi
theta_max = np.pi/2 - 20/180*np.pi
n_sigma = 40
n_theta = 25
#set initial time
t = 0
...
def u_func(x,y,sigma,theta,c,t):
    ...
    F_std = 0.04
    F_peak = 0.1
    Dir_mean = 60.0 #mean direction in degrees
...
if rank ==0:
    ...
    vals_out[:,0] = vals[:,0]
    vals_out[:,1] = Dir_vals[:,0]
    np.savetxt(out_dir+"Stations/"+"Currents_stations_SUPG_structured.csv", np.append(stats, vals_out, axis=1), delimiter=",")
