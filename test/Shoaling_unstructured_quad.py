...
fname = 'Shoaling_unstructured_quad/solution'
...
HS.vector.setValues(dofs1,np.array(HS_vec))
HS.vector.ghostUpdate()
fname = 'Shoaling_unstructured_quad_HS/solution'
xdmf = io.XDMFFile(domain1.comm, output_dir+'Paraview/'+fname+".xdmf", "w")
xdmf.write_mesh(domain1)
xdmf.write_function(HS)
xdmf.close()
...
if rank ==0:
    ...
    vals_out[:,0] = vals[:]
    np.savetxt(output_dir+'Stations/HS_stations_SUPG_unstructured_quad.csv', np.append(stats, vals_out, axis=1), delimiter=",")
